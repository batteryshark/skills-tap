...
4. Prioritize correctness hazards, hidden failure behavior, missing behavioral coverage, and high-cost comprehension problems before cosmetic consistency.
5. In cleanup mode, follow [`references/safe-refactoring.md`](references/safe-refactoring.md). Preserve public APIs, serialized shapes, persisted data, and externally observable behavior unless migration is explicit.
6. Apply the repository's language conventions. Read [`references/language-guidance.md`](references/language-guidance.md) only for the languages present.
7. Run the narrowest relevant tests, type checks, linters, formatters, or build commands.
...
Use [`agents/auditor.md`](agents/auditor.md) for an independent review and [`agents/fixer.md`](agents/fixer.md) for a bounded cleanup pass.
